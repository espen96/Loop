...
vec2 drs(){
	...
	float frametimes = abs(clamp(texture2D(colortex8,vec2(1)).r,0.0,10.0));
	
 scale = vec2(clamp(1-frametimes,0.5,1));
   ...
   #define RENDER_SCALE_X 1.0*drs().x 
   #define RENDER_SCALE_Y 1.0*drs().y  
