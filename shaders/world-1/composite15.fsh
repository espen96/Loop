...
void main() {
  ...
  float vignette = (1.5-dot(texcoord-0.5,texcoord-0.5)*2.);
	...
	float lightScat = clamp(BLOOM_STRENGTH*0.1*pow(exposure.a,0.2),0.0,1.0)*vignette;

  float VL_abs = texture2D(colortex7,texcoord).r;
	float purkinje = rodExposure/(1.0+rodExposure)*Purkinje_strength;
  VL_abs = clamp((1.0-VL_abs)*N_BLOOMY_FOG*1.5*(1.0-purkinje),0.0,1.0)*clamp(1.0-pow(cdist(texcoord.xy),15.0),0.0,1.0);
  ...
  float lum = dot(col,vec3(0.15,0.3,0.55));
	...
	col = mix(lum*(Purkinje_Multiplier/2)*vec3(Purkinje_R, Purkinje_G, Purkinje_B)+1.5e-3, col, rodCurve);
//	col =vec3(rodCurve);
