...
(noisetex, pos/32.0/1.74/1.74 + movement).bb*2.0-1.0;
    float wave = texture
(gaux3, (pos*vec2(3., 1.0)/128. + movement + displ/128.0)*exp(i*1.0)).a;
...
vec3 getWaveHeight(vec2 posxz, float iswater){
	...
	mat2 rotationMatrix  = mat2(vec2(cos(radiance),  -sin(radiance)),  vec2(sin(radiance),  cos(radiance)));
	vec2 displ = texture
(noisetex, pos/32.0 + movement).bb*2.0-1.0;
	for (int i = 0; i < 4; i++){
		vec2 displ = texture
(noisetex, pos/32.0/1.74/1.74 + movement).bb*2.0-1.0;
    vec3 wave = texture
(gaux3, (pos*vec2(3., 1.0)/128. + movement + displ/128.0)*exp(i*1.0)).rgb;
