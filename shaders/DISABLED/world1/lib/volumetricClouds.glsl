...
(noisetex,samplePos.xz/12500.).r-0.2+0.4*0.6,0.0,1.0)/(1.0-0.2+0.4);
...
//Erode cloud with 3d Perlin-worley noise, actual cloud value
	...
	float cloudVolLQ(in vec3 pos){
		...
		float coverage = clamp(texture
(noisetex,samplePos.xz/12500.).r-0.2+0.4,0.0,1.0)/(1.0-0.2+0.4);
...
(noisetex,pos2D.yx/6. ).b-0.7+0.7,0.0,1.0);
	float cloud = texture
(noisetex, pos2D).r;
	float weights = 1.0;
