...
void main() {
	...
	gl_FragData[0] = texture2D(texture, lmtexcoord.xy);
		...
		float exposure = (texelFetch2D(gaux1,ivec2(10,37),0).r+texelFetch2D(gaux1,ivec2(10,37),0).r*texelFetch2D(gaux1,ivec2(14,37),0).r*5.0);

		vec3 col = albedo/exposure*0.2;


		gl_FragData[0].rgb = col;
		gl_FragData[0].a *= 0.2*color.a;
