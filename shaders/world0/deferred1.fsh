...
#define HQ_CLOUDS           // Renders detailed clouds for viewport
...
#ifdef VOLUMETRIC_CLOUDS
    vec2 halfResTC = vec2(floor(gl_FragCoord.xy) / CLOUDS_QUALITY / RENDER_SCALE + 0.5 +
                          offsets[framemod8] * CLOUDS_QUALITY * RENDER_SCALE * 0.5);

    vec3 fragpos = toScreenSpace(vec3(halfResTC * texelSize, 1.0));
    vec4 currentClouds = vec4(0.0);

    //	if(checker <0.5)	currentClouds = renderClouds(fragpos,vec3(0.),
    //blueNoise(),sunColor/150.,moonColor/150.,avgAmbient/150.);
