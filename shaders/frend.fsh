...
float getWaterHeightmap(vec2 posxz, float iswater) {
  ...
  float moving = clamp(iswater*2.-1.0,0.0,1.0);
    ...
    float wave = texture(noisetex, (pos*vec2(3., 1.0)/128. + movement + displ/128.0)*exp(i*1.0)).b;
...
/* RENDERTARGETS: 7,3 */
  ...
  vec4 normal2 = (texture(colortex10, texcoord));
  vec4 normal3 = (texture(colortex11, texcoord));
  vec3 worldnormal = vec3(normal3.r); 

	    float sigma = 0.25;
	    float intensity = exp(-sigma * texture(colortex2,texcoord).a);  
  ...
  float refraction2 = pow(texture(colortex2,texcoord).a,3)*2-0.2;
