...
float cloudCov(in vec3 pos,vec3 samplePos){
	...
	float coverage = clamp(texture(noisetex,samplePos.xz/12500.).r-0.2+0.4*0.6,0.0,1.0)/(1.0-0.2+0.4);
...
//Erode cloud with 3d Perlin-worley noise, actual cloud value
	...
	float cloudVolLQ(in vec3 pos){
		...
		float coverage = clamp(texture(noisetex,samplePos.xz/12500.).r-0.2+0.4,0.0,1.0)/(1.0-0.2+0.4);
...
float phaseg(float x, float g){
    float gg = g * g;
    return (gg * -0.25 /3.14 + 0.25 /3.14) * pow(-.0 * (g * x) + (gg + 1.0), -1.5);
}
...
float cirrusClouds(vec3 pos){
	...
	float cirrusMap = clamp(texture(noisetex,pos2D.yx/6. ).b-0.7+0.7,0.0,1.0);
...
vec4 renderClouds(vec3 fragpositi, vec3 color,float dither,vec3 sunColor,vec3 moonColor,vec3 avgAmbient) {
		...
		vec3 sunContribution = mieDay* vec3(0.2,0.1,0.1);	
		vec3 moonContribution = mieNight*moonColor*3.14;
		vec3 skyCol0 = avgAmbient*(1.0-rainStrength*0.1);
		...
		vec3 samplePos = curvedPos*vec3(1.0,1./32.,1.0)/4+frameTimeCounter*vec3(0.5,0.,0.5)*25.;
			...
			if (coverageSP>0.00){
				...
				if (cloud > 0.0005){
					...
					float sunShadow = max(exp2(-shadowStep*muEshD),exp2(-0.25*shadowStep*muEshD))*(1.0-exp(-muE*50.0*2.0));
