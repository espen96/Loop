...
//		  noise = fract(frameCounter * (1.0 / 9.0) + noise);
	...
	if(giDistanceMask <=0.0) return vec3(0.0);	
...
//	for (int i = 0; i < steps; ++i){
	for (float i = 1.0; i < 2.0; i += steps){   //Faster but needs some extra denoising
				...
				vec4 shadowSample = vec4(texelFetch2D(shadowcolor0,ivec2(GIprojectedShadowPosition3.xy),0).rgb,texelFetch2D(shadow,ivec2(GIprojectedShadowPosition3),0).x);
				...
				if(salbl/albl >=0.98  && salbl/albl <=1.02) break;				
