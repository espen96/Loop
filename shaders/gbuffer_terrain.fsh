...
vec3 FindNormal(sampler2D tex, vec2 uv, vec2 u) {
	...
	if (vtexcoord.x > 1.0 - 0.01 || vtexcoord.y > 1.0 - 0.01)
		return vec3(0.0);
	if (vtexcoord.x < 0.01 || vtexcoord.y < 0.01)
		return vec3(0.0);
...
void main() {
	...
	mat3 tbnMatrix = mat3(tangent.x, tangent2.x, normal.x, tangent.y, tangent2.y, normal.y, tangent.z, tangent2.z, normal.z);
